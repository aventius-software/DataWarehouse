...
CREATE FUNCTION [Chronological].[_fnGetBankHolidayDateForBoxingDay] (
	@year INT
) 
RETURNS DATE AS
BEGIN
    ...
    RETURN CASE
		...
		-- Obviously I could have put the previous 'WHEN' statement and this one together
		-- but I'm just being overly 'clear' here for logic purposes
        ...
        -- taken as a bank holiday for Christmas, so Tuesday becomes a bank holiday
        -- for Boxing day. If we add 2 days to Christmas day (Sunday), we'll get the
		-- Tuesday date for our bank holiday
        WHEN DATENAME(WEEKDAY, @christmasDay) = 'Sunday' THEN DATEADD(DAY, 2, @christmasDay)

        -- Otherwise if we've reached this point, Christmas and Boxing day are just normal 
		-- week days, so our Boxing day bank holiday date is just the day after the Christmas 
		-- day - simple!
